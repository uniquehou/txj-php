...
<<<<<<< HEAD
    ...
    file_name = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/static/file/', request.session.get('name') + '.xls')
    excel_file = open(file_name, 'w')
    excel_file.write( str("\t") + str("\t").join(['周一', '周二', '周三', '周四', '周五', '周六', '周日']) + '\n')
    for i in [i for j in range(1,6) for i in range(j, 36, 5)]:
        # 表头
        if i==1:
            excel_file.write('上午\t')
        elif i==3:
            ...
            excel_file.write('晚自习\t')
        elif i<5:
            excel_file.write('\t')

        course = "course_%d" % i
        excel_file.write( free_course[course] + '\t' )

        # 换行
        if i>30:
            excel_file.write('\n')
    return HttpResponseRedirect('/txj/%s' % file_name)
=======
